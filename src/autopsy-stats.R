...
autopsies <- 
  readxl::read_excel(here("data", "2023.10.28-2023.11.27_autopsies.xls"))
...
writexl::write_xlsx(block_stain_counts, here("output", "2023-11_autopsy-counts.xlsx"))

# Stains performed for each autopsy
stains |> 
  filter(str_detect(`Result ID`, "^AU")) |> select(`Result ID`, stain) |> 
  writexl::write_xlsx((here("output", "2023-11_autopsy-stains.xlsx")))
